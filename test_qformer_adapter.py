...
from my_model.qformer_spm_q_adapter import Base
...
def predict_logits(model, dataset, config):
    ...
    out_dict={}
    def get_keys_from_values(dictionary, search_values):
        keys = []
        for value in search_values:
            for key, val in dictionary.items():
                ...
                keys.append(None)  # 如果未找到匹配的值，添加None或其他你认为合适的值
        return keys
    with torch.no_grad():
        for idx, data in tqdm(
            ...
            attr_idx, obj_idx = pairs[:, 0].cpu().numpy(), pairs[:, 1].cpu().numpy()
            batch_attr= data[1].cuda()
            batch_obj =data[2].cuda()
            attr_names = [attr + " object" for attr in get_keys_from_values(attr2idx, batch_attr)]
            obj_names=get_keys_from_values(obj2idx,batch_obj)
            # pair_names=[attr+" "+obj for attr in get_keys_from_values(attr2idx, batch_attr) for obj in get_keys_from_values(obj2idx,batch_obj)]
            pair_names=[attr+" "+obj for attr,obj in dataset.pairs]
            ...
            # logits=comp_logits
            # logits1 = torch.zeros_like(logits)
            # logits2=torch.zeros_like(logits)
            # for i in range(len(obj_idx)):
            #     idx = obj_idx[i]
            #     logits1[:, i] = obj_logits[:, idx]
            # for i in range(len(attr_idx)):
            #     idx = attr_idx[i]
            #     logits2[:, i] = attr_logits[:, idx]
            # logits = logits+logits1*logits2
            logits = logits.cpu()
            # pair_idx = torch.argmax(logits, dim=1).cpu().numpy()
            # for idx in pair_idx:
            #     predict_pair.append(pairs_dataset[idx])
            # for idx in data[3].cpu().numpy():
            #     pair_truth1.append(pairs_dataset[idx])
            all_logits = torch.cat([all_logits, logits], dim=0)
            all_attr_gt.append(attr_truth)
            all_obj_gt.append(obj_truth)
            all_pair_gt.append(pair_truth)
    # out_dict['pair_truth']=pair_truth1
    # out_dict['predict_pair']=predict_pair
    # out_dict['obj_truth']=obj_truth1
    # out_dict['predict_obj']=predict_obj
    # out_dict['attr_truth']=attr_truth1
    # out_dict['predict_attr']=predict_attr
...
if __name__ == "__main__":
    ...
    if config.open_world and config.threshold is None:
        ...
        with torch.no_grad():
            ...
            # pd.DataFrame(out_dict).to_csv(os.path.join(config.save_path, f"epoch0.csv"))
