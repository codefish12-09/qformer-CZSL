...
from my_model.qformer_adapter2 import Base
from parameters import parser, YML_PATH
from loss import loss_calu

# from test import *
import test_qformer_adapter as test
...
def train_model(model, optimizer, config, train_dataset, val_dataset, test_dataset):
    ...
    for i in range(config.epoch_start, config.epochs):
        ...
        for bid, batch in enumerate(train_dataloader):
            ...
            attr_names=[attr for attr in get_keys_from_values(attr2idx, batch_attr)]
            attr_obj_names = [attr + " object" for attr in get_keys_from_values(attr2idx, batch_attr)]
            # attr_names=[[attr] for attr in attr_names]
            obj_names=get_keys_from_values(obj2idx,batch_obj)
            pair_names=[attr+" "+obj for attr,obj in train_dataset.train_pairs]
            # obj_names=[[obj] for obj in obj_names]
...
if __name__ == "__main__":
    ...
    load_args(YML_PATH['bert'], config1)
    print(config)
    # set the seed value
